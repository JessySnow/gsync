...
// GetReleaseNeedDownLoad get a list of release that not in index
func GetReleaseNeedDownLoad(repo *config2.Repo, release *github.Release) (releases *[]github.Release, err error) {
	allReleases, err := github.ListRelease(repo.Owner, repo.Name)
	if err != nil {
		return nil, fmt.Errorf("list repo's release failed: %v", err)
	}
	if 0 == len(*allReleases) {
		return allReleases, nil
	}

	firstRelease := (*allReleases)[0]
	key, err := GenerateReleaseKey(repo, &firstRelease)
	if err != nil {
		return nil, fmt.Errorf("generate first release key filed: %v", err)
	}
	leaf, err := findLeaf(key)
	if err != nil {
		return nil, fmt.Errorf("search left most release in index failed: %v", err)
	}
	if leaf == nil {
		return allReleases, nil
	}
	...
	releaseKeys := make([]int, len(*allReleases))
	for _, r := range *allReleases {
		k, err := GenerateReleaseKey(repo, &r)
		if err != nil {
			return nil, fmt.Errorf("generate first release key filed: %v", err)
		}
	...
	needDownload := make([]github.Release, 0)
